...
COXA_SWING_GAIN = [1.0, 1.2, 1.0, 1.2, 1.0, 1.2]  # genap diperbesar

start_time = time.time()

# === MODIFIKASI 1: Variabel global untuk menyimpan sudut ===
leg_angles = [{'coxa': 0.0, 'femur': 0.0, 'tibia': 0.0} for _ in range(6)]
...
def trajectory(t, p, phase, step_height, step_duration, vx, vy, v_rot, leg_index):
    ...
    # Tambahkan gain khusus per kaki (kuatkan genap)
...
def joint_control(joint_index, pos):
    ...
    # === MODIFIKASI 2: Simpan sudut ke variabel global, jangan di-print di sini ===
    leg_angles[leg]['coxa']  = math.degrees(coxa_servo)
    leg_angles[leg]['femur'] = math.degrees(femur_servo)
    leg_angles[leg]['tibia'] = math.degrees(tibia_servo)
    # === MODIFIKASI SELESAI ===
...
def update_robot():
    while running:
        try:
            ...
            # === MODIFIKASI 3: Cetak semua sudut kaki di sini ===
            # Membersihkan layar terminal (bekerja di Linux/macOS/Windows 10+)
            print("\033[H\033[J", end="")
            print("--- Sudut Servo Aktual ---")
            for i, angles in enumerate(leg_angles):
                print(f"[Leg {i}] Coxa: {angles['coxa']:>5.1f}°, "
                      f"Femur: {angles['femur']:>5.1f}°, "
                      f"Tibia: {angles['tibia']:>5.1f}°")
            # === MODIFIKASI SELESAI ===
